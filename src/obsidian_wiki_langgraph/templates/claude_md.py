...
Ported from obsidian-llm-wiki src/templates/claude-md.ts
"""

from __future__ import annotations


def page_type_desc(page_type: str) -> str:
    """Return a human-readable description for a wiki page type."""
    ...
    return descriptions.get(page_type, page_type)


def generate_claude_md(config: dict) -> str:
    """Generate the content of CLAUDE.md for the wiki vault.

    Parameters
    ----------
    config : dict
        Must contain keys: vault_name (str), wiki_root (str), page_types (list[str]).

    Returns
    -------
    str
        The full markdown content for CLAUDE.md.
    """
    vault_name: str = config["vault_name"]
    wiki_root: str = config["wiki_root"]
    page_types: list[str] = config["page_types"]

    page_type_lines = "\n".join(
        f"- **{t}/**: {page_type_desc(t)}" for t in page_types
    )

    return f"""# Wiki 操作规范 — {vault_name}
