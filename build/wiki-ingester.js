...
class WikiIngester {
    ...
    linkEngine;
    wikiManager;
    constructor(client, linkEngine, wikiManager) {
        this.client = client;
...
exports.WikiIngester = WikiIngester;
/**
 * 构建摄入 prompt
 */
function buildIngestPrompt(sourceName, sourceContent, existingSummaries, existingPages) {
 ...
 * 确保内容有 frontmatter
 */
function ensureFrontmatter(content, fm) {
    ...
    const fmLines = Object.entries(fm)
        .map(([k, v]) => {
        if (Array.isArray(v)) {
            return `${k}: [${v.map((i) => i.includes(" ") ? `"${i}"` : i).join(", ")}]`;
        }
        return `${k}: ${v}`;
    })
        .join("\n");
...
//# sourceMappingURL=wiki-ingester.js.map