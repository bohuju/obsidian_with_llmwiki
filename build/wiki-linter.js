...
class WikiLinter {
    wikiManager;
    linkEngine;
    constructor(wikiManager, linkEngine) {
    ...
    lint() {
        ...
        const allFiles = (0, utils_js_1.walkMarkdownFiles)(wikiBase, wikiBase);
        ...
        for (const [file, content] of fileContents) {
            const links = link_engine_js_1.LinkEngine.extractLinks(content);
            for (const link of links) {
                // 只检查 wiki/ 内部的链接
                const normalizedLink = link
                    .replace(/^wiki\//, "")
                    .replace(/^\\.\\./g, "");
                if (!allWikiPaths.has(normalizedLink) &&
